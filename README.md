...
## Sumário
1. [Overview](#overview)
2. [Basic Architecture and Terminology](#basic-architecture-and-terminology)
...
4. [Query Optimization with Indexes](#query-optimization-with-indexes)
   - [The path of a Query](#the-path-of-a-query)
   - [B-Tree Index](#b-tree-index)
   - [Hash Index](#hash-index)
   - [BRIN (Block Range Index) Index](#brin-block-range-index-index)
   - [Indexes Maintenance and Best Practices](#indexes-maintenance-and-best-practices)
...
- **psql** é a CLI para interagir com o SGBD
- bom para saber os comandos pois ele tem auto complete

## Basic Architecture and Terminology
- **Cluster**: é um postgres em um server, onde temos usuários permissões
- **Database**:  é o armazenamento onde o seu arquivo estará com tabelas e indicies
- **User**: quem vai interagir e possui permissões
- o PostgreSQL utiliza um arquitetura do tipo **Client-Server**  onde o cliente estabelece sua conexão com o servidor de banco de dados
...
- Postgres não vem com um pool de connections por default, você pode utilizar o **PgBouncer**
A memória no PostgreSQL pode ser dividido em duas categorias, **Shared Memory** e **Local Memory**
- **Local**: Usado para um unico usuario
- **Shared**: Compartilhado com todos server process
- **Memory: Shared Buffers**: um dos topicos mais importantes do postgres, ele mantem uns dados na memoria para busca rapidas, por default é 128MB, é possivel ver ele pelo comando `show shared_buffers`. Resumindo um "Cache", um exemplo é quando voce executa a query segunda vez e vai muito mais rapido
- **Local Memory**: é alocado para cada server process, ele tem algumas areas como `work_mem` que é usado para sort e hash table (default 1MB), a segunda area é a `temp_buffers` onde armazena tabelas temporarias (default 8MB) e a terceira area é a `maintenance_work_mem` onde fica o [vacuum](https://www.postgresql.org/docs/current/sql-vacuum.html) e operacões de criação de indicies. 
- **WAL: Write Ahead Log** é um log de todas as mudanças de tabelas e indicies, ela é usada em caso de desastres com o banco de dados, mas tambem uma forma de voce replicar os dados para suas replicas. Após configurar o servidor das replicas em seus arquivos de configuração ele envia via TCP o WAL para as replicas. Replicações podem ser assincronas e sincronas (com locks de recurso até a confirmação da replicação), e podemos ter replicações logicas.
...
- **schemas** são basicamente namespaces dentro de um banco de dados, o default é chamado de `public`. Voce pode ter tabelas,views diferentes para cada schema
- **database objects** são basicamente tabelas, views, sequences e por ai vai...
- **search path** é uma lista de schemas que o banco de dados vai procurar data objects que não foram referenciados na query por um schema, por exemplo `SELECT * FROM xpto` ele vai usar o search path para saber qual schema esta essa tabela xpto
...
- utitlizar **unique constraint** para dados que não podem ser duplicados, o Postgres cria por baixo dos panos um b-tree index para verificar se essa unique constraint ja existe em outro lugar
...
- **role** é um jeito de agrupar os usuarios, e poder ter priveleges para essa role por exemplo
...
- Os passos para executar uma query são: **connection, parser, rewrite system, planner/optimizer e executor**

![patg](./images/patg.png)

- Dando um zoom in no **Query Optimizer**
...
- Agora dando um zoom in no **Exectution Plan**
